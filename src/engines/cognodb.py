"""
CognoDB Cloud Adapter (connected via official Neo4j Bolt driver).
Reads connection parameters strictly from environment variables or explicit config.
"""
...
class CognoDBAdapter(BaseGraphAdapter):
    def __init__(self, uri: Optional[str] = None, user: Optional[str] = None, password: Optional[str] = None, **kwargs):
        resolved_uri = uri or os.getenv("COGNODB_URI", "")
        resolved_user = user or os.getenv("COGNODB_USER", "cognodb")
        resolved_password = password or os.getenv("COGNODB_PASSWORD", "")
        super().__init__("CognoDB Cloud", resolved_uri, 7687, resolved_user, resolved_password, **kwargs)
        self.uri = resolved_uri
        self.driver = None

    def connect(self) -> None:
        if not self.uri or not self.password:
            raise ValueError("CognoDB connection requires COGNODB_URI and COGNODB_PASSWORD environment variables or arguments.")
    ...
    def clean_db(self) -> None:
        ...
        with self.driver.session() as session:
            try:
                ...
                session.run("CALL { MATCH (n) DETACH DELETE n } IN TRANSACTIONS OF 1000 ROWS").consume()

    def create_indices(self) -> None:
        print(f"[{self.name}] Creating indices...")
        with self.driver.session() as session:
            try:
                session.run("CREATE INDEX paper_id_idx IF NOT EXISTS FOR (p:Paper) ON (p.id)").consume()
            except Exception:
                try:
                    ...
                    pass
            try:
                session.run("CREATE INDEX paper_cat_idx IF NOT EXISTS FOR (p:Paper) ON (p.category)").consume()
            except Exception:
                try:
    ...
    def get_resource_footprint(self) -> Dict[str, Any]:
        return {
            "instance_specs": "Burstable 0.5 vCPU, 256 MB RAM, 1 GB Storage",
            "memory_mb": "not observable (cloud managed)",
            "disk_storage_mb": "not observable (cloud managed)"
        }
